...
form.addEventListener(
    "submit",function(e){
        ...
        let expenditure =Number(document.getElementById("expenditure").value)
        
        if( basic_salary&& benefits && expenditure){
            ...
            let remnant=calc_remnant(net_pay,expenditure)
            document.getElementById("Remnant").innerText=remnant
...
function calc_remnant(net_pay,expenditure){
        let rem=(net_pay-expenditure)
        if (rem<1){
            alert("check on your expenditure")}
        return rem
